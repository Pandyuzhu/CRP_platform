...
def print_header():
    ...
    print(f"{Fore.CYAN}{Style.BRIGHT}               登高车 ESP32 控制板模拟器")
    print(f"{Fore.CYAN}{Style.BRIGHT}===========================================================")
    print(f"{Fore.YELLOW}登高车 (IP: 192.168.3.130, 端口: 5300)")
    print(f"{Fore.CYAN}{Style.BRIGHT}===========================================================")
...
def add_command_log(cmd_bytes):
    ...
    # 构建日志条目
    log_entry = f"{timestamp} | 使能: {enable} | 关节1: {joint1} | 关节2: {joint2} | 关节3: {joint3}"
...
def udp_server(host, port):
    ...
    while running:
        try:
            ...
            print(f"{Fore.CYAN}[登高车模拟器] 收到来自 {addr} 的数据: {data.hex()}")
...
def main():
    ...
    print(f"{Fore.YELLOW}[登高车模拟器] 已退出")
